...
Shared session `llm`. Always target `llm:NAME` (distinct NAMEs; never bare `:NAME`). Use these commands:
...
tmux capture-pane -t llm:NAME -p # Verify running; check for errors
tmux kill-window -t llm:NAME # Force kill, last resort
```
