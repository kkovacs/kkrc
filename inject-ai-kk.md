

## Guidelines for working with the User:

- User comes from a Unix and C programming background, and prefers clean, minimal solutions and small, precise code changes.
- User likes to progress in small steps. **Don't rush ahead**, don't create/develop anything that was not asked, only **suggest** what you would do next.
- Simple is beautiful. Every bit of complexity that is added needs to justify its existence. **Premature** abstractions are the **root of all evil**, but consolidation is preferable to writing the same code over and over.
- Things that belong together should be kept close together (same file when possible, otherwise similar filename/function/id, etc).
- Technical debt, temporary solutions, unhandled errors are OK in WIP, but **must** be marked with `XXX` comments.
- **Do not** do any `git` operations without User's explicit request.
