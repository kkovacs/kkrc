...
function AI() { python3 -c "import sys,json,urllib.request,os;u='https://openrouter.ai/api/v1/chat/completions';h={'Authorization':'Bearer '+os.environ.get('OPENROUTER_API_KEY',''),'Content-Type':'application/json'};d=json.dumps({'model':'${2:-google/gemini-3-flash-preview}','messages':[{'role':'system','content':'$1'},{'role':'user','content':sys.stdin.read()}]}).encode();o=urllib.request.build_opener(urllib.request.ProxyHandler());print(json.loads(o.open(urllib.request.Request(u,data=d,headers=h)).read())['choices'][0]['message']['content'])" "$@"; }
export -f AI
