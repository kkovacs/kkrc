...
function s() { command -v tmux >/dev/null 2>&1 && { tmux set-environment -g SSH_AUTH_SOCK "$SSH_AUTH_SOCK"; tmux attach-session ${1:+-t} "$1" || tmux new-session ${1:+-s} "$1"; } || { screen ${1:+-S} $1 -X register s " export SSH_AUTH_SOCK=\"$SSH_AUTH_SOCK\""; screen -xR "$1"; }; }
