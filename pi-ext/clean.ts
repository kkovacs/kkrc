...
function compact(msg: Record<string, unknown>): {
    text: string;
    isToolOnly: boolean;
    ...
    switch (role) {
        ...
        case "bashExecution": {
            ...
            const outputLength = fullOutputPath
                ? DEFAULT_MAX_BYTES
                : output
...
export default function (pi: ExtensionAPI) {
    pi.registerCommand("clean", {
        description:
            "Start a fresh session with compacted context in the editor (zero LLM)",
        handler: async (_args, ctx) => {
            ...
            if (!summary) {
                ctx.ui.notify(
                    "Nothing to clean (no serializable messages)",
                    "error",
                );
                return;
            }
